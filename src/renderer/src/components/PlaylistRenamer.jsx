			...
			{editingSelectedPlaylistName ? (
				<div className="flex flex-row w-full gap-2">
					<input
						...
						onChange={(e) =>
							setSelectedPlaylistRename(toAllowedPlaylistName(e.target.value))
						}
						onKeyDown={(e) => {
							if (e.key == "Enter") {
					...
					/>
					<div
						className="bg-green-900 hover:bg-green-800 text-green-300 rounded-lg h-10 aspect-square flex flex-col justify-center items-center border border-green-300 cursor-pointer transition ease-out duration-200"
				...
				<>
					<p
						...
						onClick={() => setEditingSelectedPlaylistName(true)}
					>
						{getPlaylistFromId(selectedPlaylist).name}
					</p>
				</>
			)}
		</>
